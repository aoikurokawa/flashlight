...
impl Flags {
    pub fn new(flags: &[&str]) -> Self {
        Self {
            ...
            only_filenames: flags.contains(&"-l"),
            match_line: flags.contains(&"-x"),
...
pub fn grep(pattern: &str, flags: &Flags, files: &[&str]) -> Result<Vec<String>, Error> {
    ...
    for file_name in files {
        let contents = fs::read_to_string(*file_name).map_err(Error::msg)?;

        let mut lines: Vec<String> = contents
            .lines()
            .enumerate()
            .filter(
                |(_, line)| match (flags.case_insensitive, flags.match_line, flags.inverted) {
                    ...
                    (false, true, true) => line.to_lowercase() != pattern.to_lowercase(),
                    _ => line.contains(pattern),
                },
            )
            .map(|(i, line)| match (flags.line_numbers, multiple_files) {
                (true, true) => format!("{}:{}:{}", file_name, i + 1, line),
                (true, false) => format!("{}:{}", i + 1, line),
                (false, true) => format!("{}:{}", file_name, line),
