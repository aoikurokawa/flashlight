...
impl HighScores {
    ...
    pub fn latest(&self) -> Option<u32> {
        if self.value.is_empty() {
            ...
            Some(self.value[self.value.len() - 1])
        }
    }
    ...
    pub fn personal_top_three(&self) -> Vec<u32> {
        ...
        let top_three = if value.len() < 3 {
            let (first, _second) = value.split_at(value.len());
            first
        } else {
            let (first, _second) = value.split_at(3);
            first
        };
...
#[cfg(test)]
mod tests {
    use crate::high_score::HighScores;
    #[test]
