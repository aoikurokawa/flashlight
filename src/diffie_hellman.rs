...
pub fn public_key(p: u64, g: u64, a: u64) -> u64 {
    ...
    let mut b = g.to_biguint().unwrap();
    let mut e = a.to_biguint().unwrap();
    let m = p.to_biguint().unwrap();

    b %= m.clone();
    while e > 0.to_biguint().unwrap() {
        if e.clone() % 2.to_biguint().unwrap() == 1.to_biguint().unwrap() {
            r = (r * b.clone()) % m.clone();
        }
