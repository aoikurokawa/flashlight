...
pub struct BulkAccountLoader<F, E>
where
    ...
    E: Fn(Arc<dyn std::error::Error + Send + Sync>) + Send + Sync + 'static,
{
    ...
    accounts_to_load: Arc<Mutex<HashMap<String, AccountToLoad<F>>>>,
    buffer_and_slot_map: Arc<Mutex<HashMap<String, BufferAndSlot>>>,
    error_callbacks: Arc<Mutex<HashMap<String, E>>>,
    interval_handle: Option<tokio::task::JoinHandle<()>>,
}

impl<F, E> BulkAccountLoader<F, E>
where
    F: Fn(Vec<u8>, u64) + Send + Sync + 'static,
    E: Fn(Arc<dyn std::error::Error + Send + Sync>) + Send + Sync + 'static,
{
    ...
    pub async fn add_account(&mut self, public_key: Pubkey, callback: F) -> String {
    ...
    pub async fn add_error_callback(&self, callback: E) -> String {
    ...
    async fn load(&self) {
        ...
        let account_chunks: Vec<Vec<&AccountToLoad<F>>> = accounts_to_load
            .values()
            .collect::<Vec<_>>()
            .chunks(99)
    ...
    async fn load_chunk(&self, chunk: Vec<&AccountToLoad<F>>) {
...
impl<F, E> Clone for BulkAccountLoader<F, E>
where
    F: Fn(Vec<u8>, u64) + Send + Sync + 'static,
    E: Fn(Arc<dyn std::error::Error + Send + Sync>) + Send + Sync + 'static,
{
